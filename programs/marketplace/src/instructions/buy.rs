...
impl <'info>Buy<'info>{
    pub fn send_sol(&mut self)-> Result<()>{
        ...
        let fee= (price as u128).checked_mul(self.marketplace.fee as u128)
            .ok_or(ErrorCode::MathError)?
        .checked_div(10_000)
        .ok_or(ErrorCode::MathError)? as u64;

        let maker_amount = price.checked_sub(fee).ok_or(ErrorCode::MathError)?;
    ...
    pub fn receive_rewards(&mut self)-> Result<()>{
        ...
        mint_to(
            CpiContext::new_with_signer(self.token_program.to_account_info(), MintTo{
